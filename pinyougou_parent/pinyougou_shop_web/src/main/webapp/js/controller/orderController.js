...
app.controller('orderController', function ($scope, $controller, orderService) {
    ...
    $scope.paymentTypeArr = ["微信支付","货到付款"];//支付类型
    $scope.statusArr = ["未付款","已付款","未发货","已发货","交易成功","交易关闭","待评价"];//订单状态
    //搜索
    ...
    //发货
    $scope.deliverGoods = function (orderIdStr,status,paymentType) {
       if (confirm("确认好订单信息再发货~")){
           if (status == "2" || (paymentType == "2" && status != "4")){
               orderService.deliverGoods(orderIdStr).success(function (response) {
                   alert(response.message);
                   $scope.reloadList();
               });
           }else {
