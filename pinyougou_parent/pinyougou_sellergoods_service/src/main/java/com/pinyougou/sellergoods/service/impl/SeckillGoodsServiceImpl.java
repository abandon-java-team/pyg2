...
public class SeckillGoodsServiceImpl implements SeckillGoodsService {
    ...
    @Override
    public void isCheck(Long[] ids, String status) {
        //设置状态
        TbSeckillGoods isChange = new TbSeckillGoods();
        isChange.setStatus(status);
        //设置更新时间
        isChange.setCheckTime(new Date());
        //组装更新条件
        List longs = Arrays.asList(ids);
        Example example = new Example(TbSeckillGoods.class);
        example.createCriteria().andIn("id",longs);
        //更新数据库
        seckillGoodsMapper.updateByExampleSelective(isChange, example);
    }
}
